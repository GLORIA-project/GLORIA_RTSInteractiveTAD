...
//			DeviceMount dev = (DeviceMount) rtd.getDevice();
...
//			rtd.mntPark();
	...
	private String getStatusXML() throws Exception{
		...
		if (mountStateXml != null && now.getTime() <= mountStateTimeRecovering + 5000){
			//nothing..returning the cached state...
		}else{
			mountStateXml = service.getStatusXML();
			mountStateTimeRecovering = now.getTime();
			LogUtil.severe(this, "mountStateXml::" + mountStateXml);
		}
	...
	public void mntUnpark() throws RTException {
		throw new UnsupportedOpException ("Action item");
	...
	public void mntSlewObject(String object) throws RTException {
		...
		double altitude = Config.getPropertyDouble("rt_config", "rts_altitude");
		...
		Catalogue catalogue = new Catalogue(longitude, latitude, altitude);
	...
	private void mntSlewToCoordinatesCommon(double ascension, double declination)
			throws RTException {
